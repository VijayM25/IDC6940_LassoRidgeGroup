...
bibliography: references_first.bib # file contains bibtex for references
...
##### The study is to see the effect of dynamic connectedness of random shocks in housing price among the regions  using standard vector acutoregressive model(VAR) as well as three VAR models with shrinkage effect - Elastic Net, Lasso and Ridge.
...
## The Logistic Lasso and Ridge Regression in Predicting Corporate Failure[@corp]


##### The study was to examine lasso and ridge approach in predicting corporate bankruptcy, which is of great interest to investors, creditors, borrowing firms, and governments alike, as this methods deal well with multicolinearity and display the ideal properties to minimize the numerical instability that may occur due to overfitting. 


-   Explore lasso and ridge approach, as there are empirical models using quantitative methods and distinct variable selection techniques.

-   How the author utilized lasso and ridge method for variable shrinking.

-   Its applied in hospitality industry.

<!-- -->


*Though there are many type of models in statistic and probability domains, even a type of discriminant analysis by couple of authors like Altman's Z-score. Artificial intelligence also had multiple neural network models like  the multilayer perceptron model, the Cascor method (cascade correlation) as a learning algorithm, multilayer perceptron net and support vector machines. the author wants to explore the least absolute shrinkage and selection operator (Lasso) is a variable selection technique that has been recently applied on corporate bankruptcy forecasts, since they deal well with multicolinearity and display the ideal properties to minimize the
numerical instability that may occur due to overfitting.


## Data collection
The models were employed to a dataset of 2032 non-bankrupt firms and 401 bankrupt firms belonging to the hospitality industry, over the period 2010-2012.


## Model selection

Author used the lasso and ridge approaches, since they deal well with multicolinearity and display the ideal properties to minimize the numerical instability that may occur due to overfitting. Therefore, improve prediction accuracy can be achieved. Lasso will shrink parameter estimates towards zero and, in some cases, equate parameters to be exactly zero and thus allows the exclusion of some of the variables from the model. These solutions were compared in terms of accuracy of predictions to the stepwise methods available in SPSS. . 


### Modeling and Results

The author aims to build a logistic model to predict enterprise failure, by resorting on two kinds of approaches: stepwise or best subset selection methods, and the ridge regression or the lasso, procedures less known, since they are not usually available in most commercial software. A comparison is made between those procedures.

Model with the training set with failed and good enterprises of equal sizes

|Error |Forward Wald | Backward Wald | Lasso | Ridge|
|:------|:------ |:------------|:------|:------|
|Type I error |37.65% |37.65% |38.82% |40.59% |
|Type II error |29.21% |29.10% |28.73% |25.79% |
|Overall error |29.92% |29.92% |29.57% |27.02% |

The type I error : percentage of failed enterprises predicted good by the model

The type II error : percentage of good enterprises predicted failed by the model and 
the overall error
...
Ridge regression - https://web.archive.org/web/20150616000426id_/http://aliquote.org:80/articles/slides/mva_clinres/biblio/hoerl70.pdf

lasso using R - https://www.statology.org/lasso-regression-in-r/
another ex - https://www.science.smith.edu/~jcrouser/SDS293/labs/lab10-r.html
